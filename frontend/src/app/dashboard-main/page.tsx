...
export default function ControllerDashboard() {
    ...
    const runOptimization = async () => {
        try {
            setOptimizing(true);
            const res = await fetch(`${API_BASE}/optimize/?method=${optMethod}`);
    ...
    return (
        <div className="px-6 py-8 mx-auto max-w-7xl">
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <KPITile label="Live Trains" value={kpis.total.toString()} trend={`${kpis.delayed} delayed`} />
                <KPITile label="Delayed Trains" value={kpis.delayed.toString()} />
                <KPITile label="Average Delay (min)" value={kpis.avgDelay.toString()} />
            </section>
